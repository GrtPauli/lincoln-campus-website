...
export const EventItemCard = ({ img, title, onReadMore, post_date, tag }) => {
  return (
    ...
    <section className="w-full">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {events?.map((event, index) => (
          <EventItemCard
            key={event.id || index}
            img={`${BASE_API_URL}/images/news/${event.post_image_url}`}
            title={event.post_title}
            onReadMore={() => setModal({ data: event, open: true })}
